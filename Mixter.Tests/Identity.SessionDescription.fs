...
module ``Given a handler of session events`` =
    let sessionId = SessionId.Generate()
    let userId = { Email = "clem@mix-it.fr"}
    
    [<Fact>]
    let ``When project user connected, then it returns a Session projection`` () =
    ...
    [<Fact>]
    let ``When project user disconnected, then it returns a Remove change of Session projection`` () =
